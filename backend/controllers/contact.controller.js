...
export const createContact = async(req, res) => {
    ...
    try {
        ...
        return res.status(201).json({message: "Thank You! We will contact you ASAP", newContact})
...
export const updateContact = async(req, res) => {
    ...
    try {
        ...
        const {body} = req
        const updated = await Contact.findByIdAndUpdate(id, body, {new: true, runValidators: true})

         if (!updated) return res.status(404).json({ message: "Contact not found" });

        return res.status(200).json({message: "Contact updated successfully!", updated})
    } catch (error) {
        return res.status(500).json({ message: error.message })
    }
}
